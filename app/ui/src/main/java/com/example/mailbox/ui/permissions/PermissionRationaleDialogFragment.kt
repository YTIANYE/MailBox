package com.example.mailbox.ui.permissions
...
import com.example.mailbox.activity.K9Activity
import com.example.mailbox.activity.K9Activity.Permission
    ...
    companion object {
        ...
        fun newInstance(permission: Permission): PermissionRationaleDialogFragment {
            return PermissionRationaleDialogFragment().apply {
                arguments = bundleOf(ARG_PERMISSION to permission.name)//Bundle主要用于传递数据；它保存的数据，是以key-value(键值对)的形式存在的。
            }
        }
    }
}
